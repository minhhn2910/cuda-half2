...
//for mixed version between half constants and half2 
inline __device__ __half2 operator + (const half a, const __half2 b) {
   return __hadd2(__half2half2(a), b);
 }
inline __device__ __half2 operator * (const half a, const __half2 b) {
   return __hmul2(__half2half2(a), b);
 }
inline __device__ __half2 operator - (const half a, const __half2 b) {
   return __hsub2(__half2half2(a), b);
 }
inline __device__ __half2 operator / (const half a, const __half2 b) {
	return __h2div(__half2half2(a),b);
 }
inline __device__ __half2 operator + (const __half2 a, const half b) {
   return __hadd2(a, __half2half2(b));
 }
inline __device__ __half2 operator * (const __half2 a, const half b) {
   return __hmul2(a, __half2half2(b));
 }
inline __device__ __half2 operator - (const __half2 a, const half b) {
   return __hsub2(a, __half2half2(b));
 }
inline __device__ __half2 operator / (const __half2 a, const half b) {
	return __h2div(a,__half2half2(b));
 }
