...
__global__ void Fan1(half2 *m_cuda, half2 *a_cuda, int Size, int t)
{
  ...
  if ((t+1) %2 ==0 ){
	...
	((half2*)&(m_cuda_half[(t+1)+Size*t]))[xidx] = ((half2*)&(a_cuda_half[(t+1)+Size*t]))[xidx] / __half2half2(a_cuda_half[Size*t+t]);
	  ...
	  if((Size-1-t)!=1)
		((half2*)&(m_cuda_half[(t+1+1)+Size*t]))[xidx] = ((half2*)&(a_cuda_half[(t+1+1)+Size*t]))[xidx] / __half2half2(a_cuda_half[Size*t+t]);
		if (threadIdx.x  == 0){
...
__global__ void Fan2(half2 *m_cuda, half2 *a_cuda, half2 *b_cuda,int Size, int j1, int t)
{
  ...
  half* m_cuda_half = (half*) m_cuda;
	...
	if ((t+1) %2 ==0 ){
		((half2*)&(a_cuda_half[(1+t)+Size*(yidx+t)]))[xidx] -= ((half2*)&(m_cuda_half[(1+t)+Size*t]))[xidx] * __half2half2(a_cuda_half[t+Size*(yidx+t)]);
	
		if(yidx == 0){
				((half2*)&(b_cuda_half[1+t]))[xidx] -= ((half2*)&(m_cuda_half[(1+t)+Size*(yidx+t)]))[xidx] * __half2half2(b_cuda_half[t]);
			}
	}
	else{
		if((Size-1-t)!=1){
			...
			if(yidx == 0){
				((half2*)&(b_cuda_half[1+t+1]))[xidx] -= ((half2*)&(m_cuda_half[(1+t+1)+Size*(yidx+t)]))[xidx] * __half2half2(b_cuda_half[t]);
			}
