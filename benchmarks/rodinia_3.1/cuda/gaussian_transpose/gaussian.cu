...
__global__ void Fan1(float *m_cuda, float *a_cuda, int Size, int t)
{
	...
	m_cuda[(xidx+t+1)+Size*t] = a_cuda[(xidx+t+1)+Size*t] / a_cuda[Size*t+t];
}
...
__global__ void Fan2(float *m_cuda, float *a_cuda, float *b_cuda,int Size, int j1, int t)
{
	...
	a_cuda[(xidx+1+t)+Size*(yidx+t)] -= m_cuda[(xidx+1+t)+Size*t] * a_cuda[t+Size*(yidx+t)];
	//a_cuda[xidx+1+t][yidx+t] -= m_cuda[xidx+1+t][t] * a_cuda[t][yidx+t];
	if(yidx == 0){
		//printf("blockIdx.x:%d,threadIdx.x:%d,blockIdx.y:%d,threadIdx.y:%d,blockDim.x:%d,blockDim.y:%d\n",blockIdx.x,threadIdx.x,blockIdx.y,threadIdx.y,blockDim.x,blockDim.y);
		//printf("xidx:%d,yidx:%d\n",xidx,yidx);
//		b_cuda[xidx+1+t] -= m_cuda[Size*(xidx+1+t)+(yidx+t)] * b_cuda[t];
    b_cuda[xidx+1+t] -= m_cuda[(xidx+1+t)+Size*(yidx+t)] * b_cuda[t];
	}
}
...
void ForwardSub()
{
    ...
    float *m_T, *a_T, *b_T;
    m_T = (float*) malloc(Size * Size * sizeof(float));
    a_T = (float*) malloc(Size *Size * sizeof(float));

  for(int i =0;i<Size; i++){
    for (int j =0; j<Size; j++){
        m_T[i*Size + j] = m[j*Size + i];
        a_T[i*Size + j] = a[j*Size + i];
      }
  }
	...
	// copy memory to GPU
	cudaMemcpy(m_cuda, m_T, Size * Size * sizeof(float),cudaMemcpyHostToDevice );
	cudaMemcpy(a_cuda, a_T, Size * Size * sizeof(float),cudaMemcpyHostToDevice );
  ...
  //for (t=0; t<3; t++)
	...
	cudaMemcpy(m_T, m_cuda, Size * Size * sizeof(float),cudaMemcpyDeviceToHost );
	cudaMemcpy(a_T, a_cuda, Size * Size * sizeof(float),cudaMemcpyDeviceToHost );
	cudaMemcpy(b, b_cuda, Size * sizeof(float),cudaMemcpyDeviceToHost );

  for(int i =0;i<Size; i++){
    for (int j =0; j<Size; j++){
        m[i*Size + j] = m_T[j*Size + i];
        a[i*Size + j] = a_T[j*Size + i];
      }
  }
