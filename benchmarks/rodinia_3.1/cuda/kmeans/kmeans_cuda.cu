...
//	cudaSetDevice(1);
	...
	//~ float* distance_vec_d;
	//~ float* distance_vec = (float*) malloc(npoints*nclusters*sizeof(float));
	//~ cudaMalloc((void**) &distance_vec_d, npoints*nclusters*sizeof(float));
...
printf ("%f, ",msecTotal);
	...
	//~ cudaMemcpy(distance_vec, distance_vec_d, npoints*nclusters*sizeof(float), cudaMemcpyDeviceToHost);
	//~ for (int i =npoints/2; i<npoints/2+100; i++)
				//~ printf ("%d,", membership_new[i]);//


	//~ for (int i = 0;i<npoints*nclusters/2; i++){
		//~ printf("%f, ", distance_vec[i]);
		//~ }
	//~ printf("\n");
				//,membership_new[npoints/2],membership_new[npoints-1]);
#ifdef BLOCK_CENTER_REDUCE
