...
//convert before copying dim_cpu.space_elem 
						FOUR_VECTOR_HALF_HOST* rv_cpu_half = (FOUR_VECTOR_HALF_HOST*) malloc(dim_cpu.space_mem/2);
						half_float::half* qv_cpu_half = (half_float::half*) malloc(dim_cpu.space_mem2/2);
						FOUR_VECTOR_HALF_HOST* fv_cpu_half = (FOUR_VECTOR_HALF_HOST*) malloc(dim_cpu.space_mem/2);
...
//convert back
	for(i=0; i<dim_cpu.space_elem; i=i+1){
		...
		fv_cpu[i].v = float(fv_cpu_half[i].v);
		fv_cpu[i].x = float(fv_cpu_half[i].x);
		fv_cpu[i].y = float(fv_cpu_half[i].y);
		fv_cpu[i].z = float(fv_cpu_half[i].z);
