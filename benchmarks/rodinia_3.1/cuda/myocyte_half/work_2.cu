...
int work_2(	int xmax,
	...
	float* y;
	half_float::half* y_half;
	
	fp* d_y;
	long y_mem;

	float* x;
	half_float::half* x_half;
	fp* d_x;
	long x_mem;

	float* params;
	half_float::half* params_half;
	...
	y= (float *) malloc(y_mem*2);
	y_half= (half_float::half *) malloc(y_mem);
	
	cudaMalloc((void **)&d_y, y_mem);

	x_mem = workload * (xmax+1) * sizeof(fp);
	x= (float *) malloc(x_mem*2);
	x_half = (half_float::half *) malloc(x_mem);
	cudaMalloc((void **)&d_x, x_mem);

	params_mem = workload * PARAMETERS * sizeof(fp);
	params= (float *) malloc(params_mem*2);
	params_half= (half_float::half *) malloc(params_mem);
	...
	for(i=0; i<workload; i++){
		pointer = i * (xmax+1) + 0;
		x[pointer] = 0;
		x_half[pointer] = half_float::half(x[pointer]);
	}
	
	cudaMemcpy(d_x, x_half, x_mem, cudaMemcpyHostToDevice);
	...
	for (i = 0; i<workload * (xmax+1) * EQUATIONS; i++){
		y_half[i] = half_float::half(y[i]);
		}
	
	cudaMemcpy(d_y, y_half, y_mem, cudaMemcpyHostToDevice);
	...
	for (i = 0;i< workload * PARAMETERS; i++){
		params_half[i] = half_float::half(params_half[i]);
		}	
	
	cudaMemcpy(d_params, params_half, params_mem, cudaMemcpyHostToDevice);
	...
	cudaMemcpy(x_half, d_x, x_mem, cudaMemcpyDeviceToHost);
	cudaMemcpy(y_half, d_y, y_mem, cudaMemcpyDeviceToHost);
	 ...
	 for(i=0; i<workload; i++){
		 ...
		 for(j=0; j<(xmax+1); j++){
			 ...
			 for(k=0; k<EQUATIONS; k++){
				 ...
				 printf("%.10f, ", i, j, k, y[i*((xmax+1)*EQUATIONS) + j*(EQUATIONS)+k]);
