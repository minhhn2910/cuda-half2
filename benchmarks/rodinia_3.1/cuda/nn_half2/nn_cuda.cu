...
typedef struct latLong_half2
{
  __half2 lat;
  __half2 lng;
} LatLong_half2;

typedef struct latLong_half2_host
{
  uint32_t lat;
  uint32_t lng;
} LatLong_half2_host;
...
*/
__global__ void euclid(LatLong_half2 *d_locations, __half2 *d_distances, int numRecords,__half2 lat, __half2 lng)
{
    ...
    LatLong_half2 latLong = d_locations[globalId];
    if (globalId < numRecords/2) {
...
int main(int argc, char* argv[])
{
	...
	unsigned long blocks = ceilDiv( numRecords/2, threadsPerBlock ); // extra threads will do nothing
    ...
    */
    ...
    //~ printf("%f, ",distances[i]);
		// distances[2*i] = half2high2float(distances_half[i]);
    //  distances[2*i+1] = half2low2float(distances_half[i]);
