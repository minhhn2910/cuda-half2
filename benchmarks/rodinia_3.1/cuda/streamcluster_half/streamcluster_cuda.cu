...
typedef struct {
  ...
  half cost;  /* cost of that assignment, weight*distance */
} Point_dev_half;
...
//=======================================
__device__ __half
d_dist(int p1, int p2, int num, int dim, __half *coord_d)
{
	__half retval = __float2half(0.0);
	for(int i = 0; i < dim; i++){
		__half tmp = coord_d[(i*num)+p1] - coord_d[(i*num)+p2];
		retval += tmp * tmp;
	}
	return retval;
}
...
//=======================================
__global__ void
kernel_compute_cost(int num, int dim, long x, Point_dev_half *p, int K, int stride,
					__half *coord_d, __half *work_mem_d, int *center_table_d, bool *switch_membership_d)
{
	...
	if(tid < num)
	{
		__half *lower = &work_mem_d[tid*stride];

		// cost between this point and point[x]: euclidean distance multiplied by weight
		__half x_cost = d_dist(tid, x, num, dim, coord_d) * p[tid].weight;
...
void allocDevMem(int num, int dim)
{
	...
	CUDA_SAFE_CALL( cudaMalloc((void**) &p,					  num * sizeof(Point_half)) );
...
/*typedef struct {
  ...
  Point_half* point_half =(Point_half*) malloc(points->num*sizeof(Point_half));
  for(int i = 0;i <points->num; i++){
      point_half[i].weight = half_float::half(points->p[i].weight);
      point_half[i].assign = points->p[i].assign;
      point_half[i].cost = points->p[i].cost;
  }
	...
	*alloc_t += (double) tmp_t;
	...
	CUDA_SAFE_CALL( cudaMemcpy(p,  point_half,				   num * sizeof(Point_half), cudaMemcpyHostToDevice) );
