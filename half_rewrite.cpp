  ...
  MyASTVisitor(Rewriter &R,ASTContext *Context) : TheRewriter(R), Context(Context), CurrentFunc(nullptr), CudaCode(false){
		...
		if (isa<BinaryOperator>(CondExpr))
		{
			...
			if (IsHalf2Expr (LHS))
				...
				if (isa<IntegerLiteral>(LHS->IgnoreImpCasts()->IgnoreCasts()->IgnoreParens())) {
					...
					leftString = FLOAT2HALF2 +to_string(intLiteral->getValue().getLimitedValue	()) + ")";
					...
					leftString = FLOAT2HALF2 +to_string(fltEvalResult) + ")";			
				}else{
			...
			if (IsHalf2Expr (RHS))
				...
				if (isa<IntegerLiteral>(RHS->IgnoreImpCasts()->IgnoreCasts()->IgnoreParens())) {
					...
					rightString = FLOAT2HALF2 +to_string(intLiteral->getValue().getLimitedValue	()) + ")";
						...
						if (APFloat::semanticsSizeInBits(floatLiteral->getValue().getSemantics()) == 64) //detect double the ugly way
							...
							fltEvalResult =  floatLiteral->getValue().convertToFloat();
					rightString = FLOAT2HALF2 +to_string(fltEvalResult) + ")";			
				}else{
			...
			switch(binaryOp->getOpcode()){
				case BO_LT:
					result = HALF2_LT + leftString + "," + rightString + ");\n"; 
				break;
				case BO_GT:
					result = HALF2_GT + leftString + "," + rightString + ");\n"; 
				break;	
				case BO_EQ:
					result = HALF2_EQ + leftString + "," + rightString + ");\n"; 
				break;								
				case BO_NE:
					result = HALF2_NE + leftString + "," + rightString + ");\n"; 
				break;	
				case BO_LE:
					result = HALF2_LE + leftString + "," + rightString + ");\n"; 
				break;
				case BO_GE:
					result = HALF2_GE + leftString + "," + rightString + ");\n"; 
				break;						
				default:
				break;		 			

			}
			return "__half2 mask_if_"+ to_string(currentIfLineNumber.first) + " = " + result;
		}
	}
	...
	void RewriteBinaryOp(SourceLocation leftLocation, SourceLocation rightLocation, BinaryOperator *E){
		...
		switch(E->getOpcode()){
			case BO_AddAssign:
			case BO_Add:
				TheRewriter.InsertText(Context->getSourceManager().getExpansionLoc(leftLocation) ,HALF2_ADD);
				break;
			case BO_SubAssign:
			case BO_Sub:
				TheRewriter.InsertText(Context->getSourceManager().getExpansionLoc(leftLocation) ,HALF2_SUB);
				break;
			case BO_MulAssign:
			case BO_Mul:
				TheRewriter.InsertText(Context->getSourceManager().getExpansionLoc(leftLocation ),HALF2_MUL);
				break;
			case BO_DivAssign:
			case BO_Div	:
				TheRewriter.InsertText(Context->getSourceManager().getExpansionLoc(leftLocation) ,HALF2_DIV);
				break;
			case BO_Assign:
	...
	//			return; 	//not literals, more complex funcall, or arrayref ... doing nothing.
		...
		TheRewriter.InsertText(Context->getSourceManager().getExpansionLoc(E->getLocStart ()), FLOAT2HALF2);
	...
	void RewriteUnaryOp(UnaryOperator * E){ //handle - (h2neg(x) call if x is not literal value)
		...
		TheRewriter.InsertText(E->getOperatorLoc(),HALF2_NEG);
		TheRewriter.InsertTextAfterToken(E->getLocEnd(), ")");
	}
	void RewriteFunctionDecl(FunctionDecl* F){
		TheRewriter.ReplaceText(F->getReturnTypeSourceRange (), "__half2" );
			...
			// if(Decl->hasAttr<CUDAConstantAttr>() || Decl->hasAttr<CUDADeviceAttr>()) return true; //currently dont attemp to rewrite global decl ?/ will do it later
			 //rewrite float into __half2
			  ...
			  if (std::find(rewrittenLocation.begin(), rewrittenLocation.end(), startLoc) == rewrittenLocation.end()){ //not found, push_back + rewrite
				...
				TheRewriter.ReplaceText(startLoc, (Decl->getType().getUnqualifiedType()).getAsString().size(), "__half2 " + string(dimension, '*'));
